...
bool m_life_deducted_from_fall = false;
bool win = false;
bool loose = false;
...
void update()
{
    ...
    float ticks = (float)SDL_GetTicks() / MILLISECONDS_IN_SECOND;
           ...
           // Check if the player has fallen below a threshold and hasn't already lost a life for this fall
           if (g_current_scene == g_levelA && g_current_scene->get_state().player->get_position().y < -10.0f
               && g_current_scene->get_state().player->get_position().x < 17.0f
               && !m_life_deducted_from_fall)
           {
               LIVES--;
               m_life_deducted_from_fall = true;
               switch_to_scene(g_levelA);
           }
           
           if (g_current_scene == g_levelB && g_current_scene->get_state().player->get_position().y < -10.0f
               && g_current_scene->get_state().player->get_position().x < 16.0f
               && !m_life_deducted_from_fall)
           {
               LIVES--;
               m_life_deducted_from_fall = true;
           ...
           if (g_current_scene == g_levelC && g_current_scene->get_state().player->get_position().y < -10.0f
               && g_current_scene->get_state().player->get_position().x < 20.0f && !m_life_deducted_from_fall)
           {
               LIVES--;
               m_life_deducted_from_fall = true;
           ...
           // Reset the fall flag once the player is back on solid ground or has respawned (e.g., Y > -10.0f)
           if (g_current_scene->get_state().player->get_position().y >= -10.0f) {
               m_life_deducted_from_fall = false;
           }
    ...
    if (g_current_scene->get_state().player->m_enemy_collision) {
        ...
        g_current_scene->get_state().player->m_enemy_collision = false;
           ...
           if (g_current_scene == g_levelA && g_current_scene->get_state().player->get_position().y < -10.0f && g_current_scene->get_state().player->get_position().x > 17.0f) switch_to_scene(g_levelB);
           
           if (g_current_scene == g_levelB && g_current_scene->get_state().player->get_position().y < -10.0f && g_current_scene->get_state().player->get_position().x > 15.0f) switch_to_scene(g_levelC);
    
    if (LIVES == 0){
        switch_to_scene(glose);
        
    }
    if (LIVES>=1 && g_current_scene == g_levelC && g_current_scene->get_state().player->get_position().y < -10.0f && g_current_scene->get_state().player->get_position().x > 20.0f){switch_to_scene(gwin);}
